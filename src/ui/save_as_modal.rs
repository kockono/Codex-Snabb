//! Save As modal: overlay de input de path para guardar buffers untitled.
//!
...
/// Renderiza el modal "Guardar como".
///
...
pub fn render_save_as_modal(
    ...
    // Altura del modal: 1 borde + 1 blank + 1 input + 1 error (condicional)
    //                   + 1 blank + 1 footer + 1 borde = 7 sin error, 8 con error.
    ...
    let block = Block::default()
        .title(Line::from(vec![
            ...
            Span::styled("Guardar como ", Style::default().fg(theme.fg_primary)),
        ]))
        .borders(Borders::ALL)
    ...
    // ── Fila 0: input de path ──
    // inner.y + 0 es la primera fila disponible
    ...
    // Se coloca en la última fila del inner area disponible
