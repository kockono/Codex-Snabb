...
pub fn render_branch_picker(f: &mut Frame, area: Rect, state: &BranchPicker, theme: &Theme) {
    if !state.visible {
        return;
    }

    // ── Calcular área del overlay ──
    // ~50% del ancho, centrado
    let overlay_width = (area.width * 50 / 100)
        .max(30)
    ...
    // 4 = borde arriba + input line + separador + borde abajo
